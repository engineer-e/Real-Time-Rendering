...
class Perlin {
    ...
    noise_vec(p){
        ...
        return this.perlin_vec_interp(this.c,u,v,w)
...
//--------------------------------------------------
   ...
   turb(p,depth){
     ...
     turb_vec(p,depth){
        ...
        for(var i=0;i<depth;i++){
           // accum.add_eq(Vec3.mul({t:weight,v:this.noise(temp_p)}))
            accum.add_eq(Vec3.mul({t:weight,v:this.noise_vec(temp_p)}))
        ...
        var mag = 1//(Math.random())^255
        var out = new Vec3(Math.abs(accum.x)*mag,Math.abs(accum.y)*mag,Math.abs(accum.z)*mag)
         ...
         for(var j=0;j<2;j++){
          for(var k=0;k<2;k++){
            var weight_v = new Vec3(u-i, v-j, w-k)
             ...
             var vec =  c[i][j][k]
             //var m = Vec3.mul({t:t,v:Vec3.dot(vec,weight_v)})
             var m = Vec3.mul({t:t,u:vec,v:weight_v})
             accum.add_eq(m)
          }   
         }   
        }
