...
class NoiseTexture extends Texture {
    ...
    value(u, v, p) {
               // return color(.5, .5, .5) * (1 + std::sin(scale * p.z() + 10 * ));
       
        var a = new Vec3(0.5, 0.5, 0.5) 
        var b = this.scale*p.z
        var c = Vec3.add(new Vec3(b,b,b),Vec3.mul({t:10, v:this.noise.turb(p, 7)}))
        //var c = Vec3.add(new Vec3(b,b,b),Vec3.mul({t:10, v:this.noise.turb_vec(p, 3)}))
        
        var d = new Vec3(1+Math.sin(c.x),1+Math.sin(c.y),1+Math.sin(c.z)) 
                        
        //return Vec3.mul({ u: this.noise.turb(p,7), v:  new Vec3(1,1,1)})
        return Vec3.mul({ u: a, v:  d})
